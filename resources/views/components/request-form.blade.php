<div class="p-4 sm:p-8 mt-2r mb-10">
    <div class="r-pos">
        ...
        <div style="background-color: #6363632e !important"  class="r-pos shadow-outline p-4 sm:p-8 mt-2r  back-drop-blur-lg back-drop-blur bg-dark color-white sm:rounded-lg font-small">
            <form method="post" action="{{ route('requests.create') }}" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    <x-input-label for="textRequest" :value="__('Tell us why we would choose you')" />
                    <textarea required placeholder="Все несерьезно написанные заявки будут отклонены" name="textRequest" id="textRequest" class="border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm width-100 height-100"></textarea>
                </div>
                ...
                <div class="mb-3 w-30">
                    <x-input-label for="season-sel" :value="__('Season')" />
                    @include('components.select', array($values = $seasons))
                </div>
